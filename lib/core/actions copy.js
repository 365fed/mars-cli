...
// 执行 mars init 初始化项目
const initProject = async (project, otherArg) => {
  // 用户人机交互选项
  let res = init()
  res.then(res1 => {
    console.log(res1,'======')
...
const createProject = async (project, otherArg) => {
  // 1.提示信息
  log.hint('oc helps you create your project, please wait a moment~');
...
const addComponent01 = async (name, dest) => {
  handleEjsToFile(name, dest, '../template/component01.vue.ejs', `${name}.vue`);
}

const addComponent02 = async (name, dest) => {
  handleEjsToFile(name, dest, '../template/component02.vue.ejs', `${name}.vue`);
}
